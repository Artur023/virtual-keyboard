const dataKey = [
  [
    // row 1
    {
      key: {
        ru: ">",
        en: "§",
      },
      shift: {
        ru: "<",
        en: "±",
      },
      code: "Backquote",
      class: "key Backquote",
      caps: { ru: "Ё", en: "`" },
    },
    {
      key: { ru: "1", en: "1" },
      shift: { ru: "!", en: "!" },
      code: "Digit1",
      class: "key Digit1",
      caps: { ru: "1", en: "1" },
    },
    {
      key: { ru: "2", en: "2" },
      shift: { ru: `"`, en: "@" },
      code: "Digit2",
      class: "key Digit2",
      caps: { ru: "2", en: "2" },
    },
    {
      key: { ru: "3", en: "3" },
      shift: { ru: "№", en: "#" },
      code: "Digit3",
      class: "key Digit3",
      caps: { ru: "3", en: "3" },
    },
    {
      key: { ru: "4", en: "4" },
      shift: { ru: "%", en: "$" },
      code: "Digit4",
      class: "key Digit4",
      caps: { ru: "4", en: "4" },
    },
    {
      key: { ru: "5", en: "5" },
      shift: { ru: ":", en: "%" },
      code: "Digit5",
      class: "key Digit5",
      caps: { ru: "5", en: "5" },
    },
    {
      key: { ru: "6", en: "6" },
      shift: { ru: ",", en: "^" },
      code: "Digit6",
      class: "key Digit6",
      caps: { ru: "6", en: "6" },
    },
    {
      key: { ru: "7", en: "7" },
      shift: { ru: ".", en: "&" },
      code: "Digit7",
      class: "key Digit7",
      caps: { ru: "7", en: "7" },
    },
    {
      key: { ru: "8", en: "8" },
      shift: { ru: ";", en: "*" },
      code: "Digit8",
      class: "key Digit8",
      caps: { ru: "8", en: "8" },
    },
    {
      key: { ru: "9", en: "9" },
      shift: { ru: "(", en: "(" },
      code: "Digit9",
      class: "key Digit9",
      caps: { ru: "9", en: "9" },
    },
    {
      key: { ru: "0", en: "0" },
      shift: { ru: ")", en: ")" },
      code: "Digit0",
      class: "key Digit0",
      caps: { ru: "0", en: "0" },
    },
    {
      key: { ru: "-", en: "-" },
      shift: { ru: "_", en: "_" },
      code: "Minus",
      class: "key Minus",
      caps: { ru: "-", en: "-" },
    },
    {
      key: { ru: "=", en: "=" },
      shift: { ru: "+", en: "+" },
      code: "Equal",
      class: "key Equal",
      caps: { ru: "=", en: "=" },
    },
    {
      key: { ru: "Backspace", en: "Backspace" },
      shift: { ru: "Backspace", en: "Backspace" },
      code: "Backspace",
      class: "key key_backspace Backspace",
      noType: true,
    },
  ],
  [
    // row 2
    {
      key: { ru: "Tab", en: "Tab" },
      shift: { ru: "Tab", en: "Tab" },
      code: "Tab",
      class: "key key_tab Tab",
      noType: true,
    },
    {
      key: { ru: "й", en: "q" },
      shift: { ru: "Й", en: "Q" },
      code: "KeyQ",
      class: "key KeyQ",
    },
    {
      key: { ru: "ц", en: "w" },
      shift: { ru: "Ц", en: "W" },
      code: "KeyW",
      class: "key KeyW",
    },
    {
      key: { ru: "у", en: "e" },
      shift: { ru: "У", en: "E" },
      code: "KeyE",
      class: "key KeyE",
    },
    {
      key: { ru: "к", en: "r" },
      shift: { ru: "К", en: "R" },
      code: "KeyR",
      class: "key KeyR",
    },
    {
      key: { ru: "е", en: "t" },
      shift: { ru: "Е", en: "T" },
      code: "KeyT",
      class: "key KeyT",
    },
    {
      key: { ru: "н", en: "y" },
      shift: { ru: "Н", en: "Y" },
      code: "KeyY",
      class: "key KeyY",
    },
    {
      key: { ru: "г", en: "u" },
      shift: { ru: "Г", en: "U" },
      code: "KeyU",
      class: "key KeyU",
    },
    {
      key: { ru: "ш", en: "i" },
      shift: { ru: "Ш", en: "I" },
      code: "KeyI",
      class: "key KeyI",
    },
    {
      key: { ru: "щ", en: "o" },
      shift: { ru: "Щ", en: "O" },
      code: "KeyO",
      class: "key KeyO",
    },
    {
      key: { ru: "з", en: "p" },
      shift: { ru: "З", en: "P" },
      code: "KeyP",
      class: "key KeyP",
    },
    {
      key: { ru: "х", en: "[" },
      shift: { ru: "Х", en: "{" },
      code: "BracketLeft",
      class: "key BracketLeft",
      caps: { ru: "X", en: "[" },
    },
    {
      key: { ru: "ъ", en: "]" },
      shift: { ru: "Ъ", en: "}" },
      code: "BracketRight",
      class: "key BracketRight",
      caps: { ru: "Ъ", en: "]" },
      // },
      // {
      //   key: { ru: '\\', en: '\\' }, shift: { ru: '|', en: '|' }, code: 'Backslash', class: 'key key_backslash Backslash', caps: { ru: '\\', en: '\\'}
      // },
      // {
      //   key: { ru: 'Del', en: 'Del' }, shift: { ru: 'Del', en: 'Del' }, code: 'Delete', class: 'key key_del Delete', noType: true,
    },
  ],
  [
    // row 3
    {
      key: { ru: "CapsLock", en: "CapsLock" },
      shift: { ru: "CapsLock", en: "CapsLock" },
      code: "CapsLock",
      class: "key CapsLock",
      noType: true,
    },
    {
      key: { ru: "ф", en: "a" },
      shift: { ru: "Ф", en: "A" },
      code: "KeyA",
      class: "key KeyA",
    },
    {
      key: { ru: "ы", en: "s" },
      shift: { ru: "Ы", en: "S" },
      code: "KeyS",
      class: "key KeyS",
    },
    {
      key: { ru: "в", en: "d" },
      shift: { ru: "В", en: "D" },
      code: "KeyD",
      class: "key KeyD",
    },
    {
      key: { ru: "а", en: "f" },
      shift: { ru: "А", en: "F" },
      code: "KeyF",
      class: "key KeyF",
    },
    {
      key: { ru: "п", en: "g" },
      shift: { ru: "П", en: "G" },
      code: "KeyG",
      class: "key KeyG",
    },
    {
      key: { ru: "р", en: "h" },
      shift: { ru: "Р", en: "H" },
      code: "KeyH",
      class: "key KeyH",
    },
    {
      key: { ru: "о", en: "j" },
      shift: { ru: "О", en: "J" },
      code: "KeyJ",
      class: "key KeyJ",
    },
    {
      key: { ru: "л", en: "k" },
      shift: { ru: "Л", en: "K" },
      code: "KeyK",
      class: "key KeyK",
    },
    {
      key: { ru: "д", en: "l" },
      shift: { ru: "Д", en: "L" },
      code: "KeyL",
      class: "key KeyL",
    },
    {
      key: { ru: "ж", en: ";" },
      shift: { ru: "Ж", en: ":" },
      code: "Semicolon",
      class: "key Semicolon",
      caps: { ru: "Ж", en: ";" },
    },
    {
      key: { ru: "э", en: `'` },
      shift: { ru: "Э", en: '"' },
      code: "Quote",
      class: "key Quote",
      caps: { ru: "Э", en: "'" },
    },
    {
      key: { ru: "ё", en: "\\" },
      shift: { ru: "Ё", en: "|" },
      code: "Backslash",
      class: "key key_backslash Backslash",
      caps: { ru: "Ё", en: "\\" }
    },
    {
      key: { ru: "Enter", en: "Enter" },
      shift: { ru: "Enter", en: "Enter" },
      code: "Enter",
      class: "key key_enter Enter",
      noType: true
    },
  ],
  [
    // row 4
    {
      key: { ru: "Shift", en: "Shift" },
      shift: { ru: "Shift", en: "Shift" },
      code: "ShiftLeft",
      class: "key key-left-shift ShiftLeft",
      noType: true,
    },
    {
      key: { ru: "]", en: "`" },
      shift: { ru: "[", en: "~" },
      code: "IntlBackslash",
      class: "key h IntlBackslash",
      noType: true,
    },
    {
      key: { ru: "я", en: "z" },
      shift: { ru: "Я", en: "Z" },
      code: "KeyZ",
      class: "key KeyZ",
    },
    {
      key: { ru: "ч", en: "x" },
      shift: { ru: "Ч", en: "X" },
      code: "KeyX",
      class: "key KeyX",
    },
    {
      key: { ru: "с", en: "c" },
      shift: { ru: "С", en: "C" },
      code: "KeyC",
      class: "key KeyC",
    },
    {
      key: { ru: "м", en: "v" },
      shift: { ru: "М", en: "V" },
      code: "KeyV",
      class: "key KeyV",
    },
    {
      key: { ru: "и", en: "b" },
      shift: { ru: "И", en: "B" },
      code: "KeyB",
      class: "key KeyB",
    },
    {
      key: { ru: "т", en: "n" },
      shift: { ru: "Т", en: "N" },
      code: "KeyN",
      class: "key KeyN",
    },
    {
      key: { ru: "ь", en: "m" },
      shift: { ru: "Ь", en: "M" },
      code: "KeyM",
      class: "key KeyM",
    },
    {
      key: { ru: "б", en: "," },
      shift: { ru: "Б", en: "<" },
      code: "Comma",
      class: "key Comma",
      caps: { ru: "Б", en: "," },
    },
    {
      key: { ru: "ю", en: "." },
      shift: { ru: "Ю", en: ">" },
      code: "Period",
      class: "key Period",
      caps: { ru: "Ю", en: "." },
    },
    {
      key: { ru: "/", en: "/" },
      shift: { ru: "?", en: "?" },
      code: "Slash",
      class: "key Slash",
      caps: { ru: "/", en: "/" },
    },
    {
      key: { ru: "▲", en: "▲" },
      shift: { ru: "▲", en: "▲" },
      code: "ArrowUp",
      class: "key ArrowUp",
    },
    {
      key: { ru: "Shift", en: "Shift" },
      shift: { ru: "Shift", en: "Shift" },
      code: "ShiftRight",
      class: "key key_right-shift ShiftRight",
      noType: true,
    },
  ],
  [
    // row 5
    {
      key: { ru: "fn", en: "fn" },
      shift: { ru: "fn", en: "fn" },
      code: "",
      class: "key fn",
      noType: true,
    },
    {
      key: { ru: "Ctrl", en: "Ctrl" },
      shift: { ru: "Ctrl", en: "Ctrl" },
      code: "ControlLeft",
      class: "key key-left-ctrl ControlLeft",
      noType: true,
    },
    {
      key: { ru: "Alt", en: "Alt" },
      shift: { ru: "Alt", en: "Alt" },
      code: "AltLeft",
      class: "key key-left-alt AltLeft",
      noType: true,
    },
    {
      key: { ru: "cmd", en: "cmd" },
      shift: { ru: "cmd", en: "cmd" },
      code: "MetaLeft",
      class: "key key-meta-left MetaLeft",
      noType: true,
    },
    {
      key: { ru: " ", en: " " },
      shift: { ru: " ", en: " " },
      code: "Space",
      class: "key key-space Space",
    },
    {
      key: { ru: "cmd", en: "cmd" },
      shift: { ru: "cmd", en: "cmd" },
      code: "MetaRight",
      class: "key key-meta-right MetaRight",
      noType: true,
    },
    {
      key: { ru: "Alt", en: "Alt" },
      shift: { ru: "Alt", en: "Alt" },
      code: "AltRight",
      class: "key key-right-alt AltRight",
      noType: true,
    },
    {
      key: { ru: "◄", en: "◄" },
      shift: { ru: "◄", en: "◄" },
      code: "ArrowLeft",
      class: "key ArrowLeft",
    },
    {
      key: { ru: "▼", en: "▼" },
      shift: { ru: "▼", en: "▼" },
      code: "ArrowDown",
      class: "key ArrowDown",
    },
    {
      key: { ru: "►", en: "►" },
      shift: { ru: "►", en: "►" },
      code: "ArrowRight",
      class: "key ArrowRight",
    },
  ],
];

export default dataKey;
